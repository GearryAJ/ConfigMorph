...
def measure(vendor,builder,tier):
    ...
    mappings.security_rule_placement=SecurityRulePlacement(mode="BOTTOM"); version="9.20" if vendor is Vendor.ASA else "7.4"
...
if __name__=="__main__":
    results={f"{v.value}/{tier}":measure(v,b,tier) for v,b in ((Vendor.ASA,asa),(Vendor.FORTIGATE,fortigate)) for tier in TIERS}
