...
class MigrationPlanner:
    def plan(self,cfg,mappings:MigrationMappings):
        ...
        if source!=Vendor.ASA and source!=Vendor.ASA.value: raise ValueError("Only Cisco ASA source migration is supported")
        ...
        for x in cfg.interfaces: add(x,"interface",S.MANUAL_REVIEW,None,"ASA interface types are mapping-only; no interface command generated.",required=[f"interface:{x.name}"] if x.name not in interface_maps else [])
        ...
        for x in cfg.services:
            if x.vendor_extensions.get("source_operator") or x.vendor_extensions.get("destination_operator"): add(x,"service",S.MANUAL_REVIEW,None,"ASA service operator cannot be represented exactly.")
        ...
        for x in sorted(cfg.security_policies,key=lambda p:p.position):
            ...
            if x.vendor_extensions.get("attached") is False: add(x,"security_policy",S.MANUAL_REVIEW,None,"ACL is not attached and is not proven active.",topology=topology)
        ...
        for x in cfg.nat_policies:
            ...
            if x.identity: add(x,"nat_policy",S.MANUAL_REVIEW,None,"Identity NAT is preserved but not rendered.")
            elif x.type not in {"static_source_nat","dynamic_pat"}: add(x,"nat_policy",S.MANUAL_REVIEW,None,f"NAT type {x.type} is preserved but not safely rendered.")
                ...
                add(x,"nat_policy",S.SUPPORTED,{"from":[zone_maps[z] for z in x.source_zones],"to":[zone_maps[z] for z in x.destination_zones],"source":resolve(x.original_source),"destination":resolve(x.original_destination or ["any"]),"service":"any","type":x.type,"translated_source":resolve(x.translated_source),"translation_target":x.translation_target,"position":x.position})
        ...
        for x in cfg.unparsed_constructs: advisories.append(f"Preserved unparsed ASA construct at line {x.line_number}: {x.reason}")
        blocked=[x.message for x in cfg.warnings if x.severity==Severity.ERROR]
        return MigrationPlan(source_vendor=Vendor.ASA,mappings=mappings,compatibility=compatibility,names=names,generate=generate,blocked=blocked,advisories=advisories)
