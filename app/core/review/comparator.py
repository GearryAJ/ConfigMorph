...
def build_review(cfg,plan,commands=(),decisions=None):
    ...
    for c in plan.compatibility:
        ...
        items.append(MigrationReviewItem(id=c.entity_id,entity_type=c.entity_type,source_id=c.entity_id,source_name=c.source_name,target_name=names.get(c.entity_id),compatibility_status=c.status,review_status=decision.status,note=decision.note,semantic_hash=digest,source_semantics=source,target_semantics=target,preserved_fields=preserved,changed_fields=changed,dropped_fields=[k for k in source if k not in target and source[k] not in (None,[],{},False)],manual_review_reasons=c.reasons,warnings=[],mapping_evidence={"required":c.required_mappings,"topology":c.topology},analysis_findings=findings[c.entity_id],generated_commands=by_command[c.entity_id],used_by=used,source_vendor=plan.source_vendor.value))
    reviewed=sum(x.review_status!=ReviewStatus.NOT_REVIEWED for x in items)
    summary=ReviewSummary(total=len(items),generated=sum(bool(x.generated_commands) for x in items),manual_review=sum(x.compatibility_status=="MANUAL_REVIEW" for x in items),unsupported=sum(x.compatibility_status=="UNSUPPORTED" for x in items),reviewed=reviewed,accepted=sum(x.review_status==ReviewStatus.ACCEPTED for x in items),needs_changes=sum(x.review_status==ReviewStatus.NEEDS_CHANGES for x in items),blocked=sum(x.review_status==ReviewStatus.BLOCKED for x in items),manual_review_remaining=sum(x.compatibility_status=="MANUAL_REVIEW" and x.review_status==ReviewStatus.NOT_REVIEWED for x in items))
    return MigrationReview(source_vendor=plan.source_vendor.value,target_vendor=plan.target_vendor.value,items=items,summary=summary)