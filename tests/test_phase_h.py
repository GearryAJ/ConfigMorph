...
def test_review_generation_hash_traceability_and_decisions():
    ...
    assert policy.target_semantics["from"]==["trust"] and policy.generated_commands and policy.generated_commands[0]["id"]
    ...
    changed=build_review(cfg,changed_plan,commands,decision); assert next(x for x in changed.items if x.id=="r0").review_status=="NOT_REVIEWED"
...
def test_validation_accounting_warning_blocking_and_zip():
    ...
    assert validation.status=="WARNING" and review.summary.generated+review.summary.manual_review+review.summary.unsupported==review.summary.total
...
def test_review_api_persistence_invalidation_validation_and_package():
    client=TestClient(app); source=Path("examples/asa/basic.cfg").read_text(); response=client.post("/api/analyze",data={"source":source,"source_vendor":"cisco_asa","target_vendor":"paloalto"}); project=response.text.split("Project: <code>")[1].split("<")[0]; base=f"/api/projects/{project}/migration"
    ...
    review=client.get(base+"/review").json(); item=next(x for x in review["items"] if x["generated_commands"])
    ...
    package=client.get(base+"/review-package"); assert package.status_code==200 and package.headers["content-disposition"]=='attachment; filename="migration-review-package.zip"'
