...
def test_basic_golden_and_no_silent_omission():
    ...
    assert lines==Path("tests/golden/migration/asa_to_pan/basic.set").read_text().splitlines()
    ...
    assert policy.status=="MANUAL_REVIEW" and "not attached" in policy.reasons[0]
...
def test_policy_order_disabled_logging_and_mapping():
    ...
    assert next(i for i,x in enumerate(lines) if " rules First " in x)<next(i for i,x in enumerate(lines) if " rules Second " in x)
    assert any(" rules Second disabled yes" in x for x in lines) and any(" rules Second log-end yes" in x for x in lines)
...
def test_migration_api_smoke_and_downloads():
    ...
    response=client.post("/api/analyze",data={"source":source,"source_vendor":"cisco_asa","target_vendor":"paloalto"}); project=response.text.split("Project: <code>")[1].split("<")[0]
